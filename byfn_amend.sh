...
function networkUp () {
  ...
  if [ "${IF_COUCHDB}" == "couchdb" ]; then
    IMAGE_TAG=$IMAGETAG CHANNEL_NAME=$CHANNEL_NAME TIMEOUT=$CLI_TIMEOUT DELAY=$CLI_DELAY LANG=$LANGUAGE docker-compose -f ${COMPOSE_FILE}${COMPOSE_FILE_ADDITIONS} -f $COMPOSE_FILE_COUCH up -d 2>&1
  else
    IMAGE_TAG=$IMAGETAG CHANNEL_NAME=$CHANNEL_NAME TIMEOUT=$CLI_TIMEOUT DELAY=$CLI_DELAY LANG=$LANGUAGE docker-compose -f ${COMPOSE_FILE}${COMPOSE_FILE_ADDITIONS} up -d 2>&1
  fi
  if [ $? -ne 0 ]; then
...
function networkDown () {
  ...
  if [ "$MODE" != "restart" ]; then
    ...
    rm -f docker-compose-e2e.yaml docker-compose-cas.yaml docker-compose-ca3.yaml
  fi
}

# Using docker-compose-e2e-template.yaml, docker-compose-cas-template.yaml and docker-compose-ca3-template.yaml - replace constants with private key file names
# generated by the cryptogen tool and output a docker-compose yaml files specific to this
# configuration
...
ADDITIONS=""
  ...
  if [ "${IF_COUCHDB}" == "couchdb" ]; then
        echo
        echo "${EXPMODE} with channel '${CHANNEL_NAME}' and CLI timeout of '${CLI_TIMEOUT}' seconds and CLI delay of '${CLI_DELAY}' seconds and using database '${IF_COUCHDB}',  ${ADDITIONS}"
  else
